...
breakSound.setAttribute("src", "break.mp3");
breakSound.muted = true;

drop.setAttribute("src", "drop.mp3");
drop.muted = true;

movesound.setAttribute("src", "movesound.mp3");
    ...
    for(let r = 0; r < shape.length; r++) {
        for(let c = 0; c < shape[r].length; c++){
            if(shape[r][c]) {
                ...
                block.setAttribute("id", `block-${row + r}-${col + c}`);  // check 
...
// erase tetromino from game board
